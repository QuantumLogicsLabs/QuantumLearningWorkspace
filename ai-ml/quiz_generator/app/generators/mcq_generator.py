...
class MCQGenerator(BaseGenerator):
    ...
    def generate(
        ...
        """
        Generate Multiple Choice Questions using the Groq API, returning
        ...
        - Clearly mark the correct answer (it must match one option exactly).
        ...
        try:
            ...
            raise RuntimeError(
                f"MCQGenerator: could not parse LLM response as JSON: {exc}"
            ) from exc
