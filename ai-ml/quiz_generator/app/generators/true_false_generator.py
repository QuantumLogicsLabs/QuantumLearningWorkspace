...
class TrueFalseGenerator(BaseGenerator):
    ...
    def generate(
        ...
        """
        Generate True/False questions using the Groq API, returning
        structured Question objects (not raw text).
        """
        prompt = f"""
        Generate exactly {number_of_questions} True/False questions
        from the following text.

        Rules:
        - Each question must have only two possible answers: "True" or "False".
        ...
        try:
            ...
            raise RuntimeError(
                f"TrueFalseGenerator: could not parse LLM response as JSON: {exc}"
            ) from exc
