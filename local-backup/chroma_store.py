...
# chatbot/.env (Docker) and the monorepo root .env (local checkout)
_CHATBOT_ROOT = Path(__file__).resolve().parent.parent
load_dotenv(dotenv_path=_CHATBOT_ROOT.parent / ".env")
load_dotenv(dotenv_path=_CHATBOT_ROOT / ".env")

# Fallback is next to the chatbot package, NOT cwd and NOT a Windows-only path.
# Relative CHROMA_DB_PATH values are resolved against this same root.
DEFAULT_CHROMA_PATH = str(_CHATBOT_ROOT / "chroma_data")
...
def resolve_chroma_path(path: str | None = None) -> str:
    raw = path or os.getenv("CHROMA_DB_PATH") or DEFAULT_CHROMA_PATH
    p = Path(raw)
    if not p.is_absolute():
        p = (_CHATBOT_ROOT / raw).resolve()
    else:
        p = p.resolve()
    return str(p)
...
def get_chroma_client(path: str | None = None):
    """One PersistentClient per process/path — Chroma does not sync two clients on the same dir."""
    global _client, _client_path
    resolved = resolve_chroma_path(path)
    if _client is None or _client_path != resolved:
...
def store_chunks(chunks: list[dict], user_id: str, document_id: str, title: str) -> int:
    ...
    scoped_user_id = (user_id or "").strip().lower()
...
def query_chunks(
    ...
    if user_id is not None:
        where["user_id"] = user_id.strip().lower()
